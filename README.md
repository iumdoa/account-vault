...
- 🛡️ **现代密码学保障 (Modern Cryptography)**
  ...
  - **安全事务落盘**：写入临时文件 -> flush + fsync -> 解密预校验成功后原子替换主库，自动保留上一份有效快照（`.previous.avlt`）。
...
- ⌨️ **全键盘流极速操作**
  - 支持名称、账号、网址/IP 片段、标签及备注的高性能模糊搜索（千条记录毫秒级检索）。
  - 支持上下键快速切换、长按滚动、一键分别复制账号与密码。
- 📦 **安全加密备份与整库恢复**
...
- [x] Argon2id 密钥派生与 AES-256-GCM 事务安全存储
- [x] 居中快捷搜索面板与完整管理视图
- [x] 加密备份导出与整库导入恢复
- [x] 键盘快捷导航与自定义快捷键 pre-check 冲突校验
